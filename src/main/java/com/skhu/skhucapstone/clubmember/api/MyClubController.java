...
public class MyClubController {
    ...
    public ResponseEntity<ApiResponse<List<MyClubResponse>>> getMyClubs(@AuthenticationPrincipal Long userId) {
    ...
    public ResponseEntity<ApiResponse<List<MyClubJoinResponse>>> getMyClubJoins(@AuthenticationPrincipal Long userId) {

        List<MyClubJoinResponse> response = clubMemberService.getMyClubJoins(userId);

        return ResponseEntity.ok(ApiResponse.success(SuccessCode.MY_CLUB_JOIN_LIST_GET_SUCCESS, response));
    }
}